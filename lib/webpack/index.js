...
class ClosureCompilerPlugin {
    ...
    compiler.plugin('compilation', (compilation) => {
      compilation.plugin('optimize-chunk-assets', (originalChunks, cb) => {
        ...
        originalChunks.forEach(chunk => {
          ...
          entryPoints.push(chunk.entryModule.fileDependencies[0]);
          entryChunkIds.add(chunkNum);
          chunkNum += ClosureCompilerPlugin.addChunksToCompilation(
              compilation, chunk, allSources, assetMap, BASE_MODULE_NAME, moduleDefs, chunkNum, requestShortener);
        });
        ...
        compilerProcess.on('close', (exitCode) => {
          ...
          outputFiles.forEach(outputFile => {
            ...
            if (entryChunkIds.has(parseInt(chunkIdParts[1], 10))) {
              source = baseFile.src + '\n' + source;
            }
  ...
  static addChunksToCompilation(compilation, chunk, sources, assetMap, baseModule, moduleDefs, nextModuleNum, requestShortener) {
    const assetName = chunk.files.find(filename => !!compilation.assets[filename]);
    const chunkSources = chunk.getModules()
        .map(webpackModule => {
          let path = webpackModule.userRequest;
          if (!path) {
            path = `__unknown_${nextModuleNum++}__`;
          }
          return {
