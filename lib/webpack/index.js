  ...
  apply(compiler) {
    ...
    compiler.plugin('compilation', (compilation) => {
      ...
      compilation.plugin('optimize-chunk-assets', (originalChunks, cb) => {
        ...
        //
        // const buffer = allSources.filter(source => /remoteplatform\/js\/editor\.js$/.test(source.path));
        // if (buffer.length > 0) {
        //   require('fs').writeFileSync('editor.js', buffer[0].src, 'utf8');
        // }
        // require('fs').writeFileSync('all_sources.json', JSON.stringify(allSources, null, 2), 'utf8');
        // const tinymce = allSources.filter(source => /tinymce\/tinymce\.js$/.test(source.path));
        // if (tinymce.length > 0) {
        //   require('fs').writeFileSync('tinymce.js', tinymce[0].src, 'utf8');
        // }
