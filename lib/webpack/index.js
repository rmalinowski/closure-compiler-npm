...
const REPLACEMENTS = {
  ...
  __webpack_public_path__: "__webpack_public_path__", // eslint-disable-line camelcase
  __webpack_modules__: "__wpcc.modules", // eslint-disable-line camelcase
  __webpack_chunk_load__: "__webpack_require__.import", // eslint-disable-line camelcase
  __non_webpack_require__: "require", // eslint-disable-line camelcase
  __webpack_nonce__: "__webpack_nonce__", // eslint-disable-line camelcase
  "require.onError": "__wpcc.onError" // eslint-disable-line camelcase
};
...
class ClosureCompilerPlugin {
  ...
  apply(compiler) {
    ...
    compiler.plugin('compilation', (compilation, params) => {
      ...
      compilation.mainTemplate.apply(new JsonpChunkTemplatePlugin());
      ...
      compilation.plugin('optimize-chunk-assets', (originalChunks, cb) => {
        ...
        runtimeModule.id = -1;
        runtimeModule.source = function() { return new RawSource('') };
        const runtimeChunk = new Chunk('__closure_compiler_runtime__');
        runtimeChunk.id = -1;
        runtimeChunk.ids = [runtimeChunk.id];
        runtimeChunk.addModule(runtimeModule);
        runtimeModule.addChunk(runtimeChunk);
        runtimeChunk.entryModule = runtimeModule;
        runtimeChunk.entrypoints[runtimeChunk.name] = new Entrypoint(runtimeChunk.name);

        const runtimeLazyModule = new Module();
        runtimeLazyModule.id = -2;
        runtimeLazyModule.source = function() { return new RawSource('') };
        const runtimeLazyChunk = new Chunk('__closure_compiler_lazy_chunk__');
        runtimeLazyChunk.id = -2;
        runtimeLazyChunk.ids = [runtimeLazyChunk.id];
        runtimeLazyChunk.addModule(runtimeLazyModule);
        runtimeChunk.addChunk(runtimeLazyChunk);

        const runtimeSource = compilation.mainTemplate.render(
            'closure-compiler', runtimeChunk, compilation.moduleTemplate, compilation.dependencyTemplates)
        // const runtimeSource = compilation.chunkTemplate.render(
        //     runtimeChunk, compilation.moduleTemplate, compilation.dependencyTemplates);

        allSources.unshift({
          path: '__webpack__base_module__',
          src: runtimeSource.source()
        });
